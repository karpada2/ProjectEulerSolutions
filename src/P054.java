...
public final class P054 implements Solution {
    ...
    public static final Combinations[] combinations = {new HighCard(), new Pair(), new TwoPair(), new ThreeOfKind(), new Straight(), new Flush(), new FullHouse(), new FourOfKind(), new StraightFlush()};

    public static boolean isPlayer1Win(Card[] player1, Card[] player2) {
        long score1 = 0;
        long score2 = 0;
        for (int i = 0; i < combinations.length; i++) {
            score1 = Math.max(score1, combinations[i].calculateScore(player1));
            score2 = Math.max(score2, combinations[i].calculateScore(player2));
        }
        return score1 > score2;
    }

    public static long getScoreOfHand(Card[] hand) {
        long score = 0;
        for (int i = 0; i < combinations.length; i++) {
            score = Math.max(score, combinations[i].calculateScore(hand));
        }
    ...
    @Override
    public String run() {
        String[] pokerHands = getHands();
        int player1Wins = 0;
        Card[] player1 = new Card[5];
        Card[] player2 = new Card[5];
        for (int i = 0; i < pokerHands.length; i++) {
            String hand = pokerHands[i];
            int index = 0;
            for (int j = 0; j < 5; j++) {
                player1[j] = new Card(hand.charAt(index), hand.charAt(index+1));
                index += 3;
            }
            for (int j = 0; j < 5; j++) {
                player2[j] = new Card(hand.charAt(index), hand.charAt(index+1));
                index += 3;
            }

            boolean player1Win = isPlayer1Win(player1, player2);

//            System.out.println(Arrays.toString(player1) + ", " + getScoreOfHand(player1));
//            System.out.println(Arrays.toString(player2) + ", " + getScoreOfHand(player2));
//            System.out.println("-----------------------------------------------------------");

            player1Wins += player1Win ? 1 : 0;
        }
    ...
    public static class Card {
        ...
        private Suit suit; //S - spades, H - hearts, C - clubs (tiltan), D - diamonds
        ...
        static final String[] values = {"", "", "2", "3", "4", "5", "6", "7", "8", "9", "T", "J", "Q", "K", "A"};
        static final String[] suits = {"S", "H", "C", "D"};


        @Override
        public String toString() {
            return values[this.value] + suits[this.suit.ordinal()];
        }
    }

    public static interface Combinations {
        public long calculateScore(Card[] arr);
    }

    public static class HighCard implements Combinations{
        @Override
        public long calculateScore(Card[] arr) {
            HashSet<Card> used = new HashSet<>(arr.length);
            long score = 0;
            for (int j = 0; j < arr.length; j++) {
                int minIndex = 0;
                int minValue = 999;
                for (int i = 0; i < arr.length; i++) {
                    if (arr[i].getValue() < minValue && !used.contains(arr[i])) {
                        minIndex = i;
                        minValue = arr[i].getValue();
                    }
                }
                used.add(arr[minIndex]);
                score += (long)(Math.pow(10, 2*(j)))*arr[minIndex].getValue();
            }
    ...
    public static class Pair implements Combinations{
        ...
        public long calculateScore(Card[] arr) {
            ...
            for (int i = 0; i < counterArray.length; i++) {
                if (counterArray[i] == 1) {
                    score += (long)(Math.pow(10, 2*(singlesCounter)))*i;
                    singlesCounter++;
                }
                if (counterArray[i] == 2) {
                    score += 1_00_00_00_00_00L + (i*1_00_00_00_00L);
                    found = true;
                }
            }
    ...
    public static class TwoPair implements Combinations{
        ...
        public long calculateScore(Card[] arr) {
            ...
            for (int i = 0; i < counterArray.length; i++) {
                ...
                if (counterArray[i] == 2) {
                    if (foundFirstPair) {
                        score += (2_00_00_00_00_00L) + (1_00_00_00_00L*i) + (1_00_00_00L*firstPairIndex);
                        found = true;
                    } else {
                        foundFirstPair = true;
                        firstPairIndex = i;
    ...
    public static class ThreeOfKind implements Combinations{
        ...
        public long calculateScore(Card[] arr) {
            ...
            for (int i = 0; i < counterArray.length; i++) {
                ...
                if (counterArray[i] == 3) {
                    score += 3_00_00_00_00_00L + (1_00_00_00_00L*i);
                    found = true;
                }
            }
    ...
    public static class Straight implements Combinations{
        @Override
        public long calculateScore(Card[] arr) {
            ...
            for (int i = counterArr.length - 1; i >= 1; i--) {
                if (counterArr[i] >= 1) {
                    ...
                    for (int k = i; k > i-5; k--) {
                        if (k < 0 || counterArr[k] < 1) {
                            isExist = false;
                        }
                    }
                    if (isExist) {
                        score += 4_00_00_00_00_00L + (1_00_00_00_00L*i) + (1_00_00_00L*(i-1)) + (1_00_00L*(i-2)) + (1_00L*(i-3)) + (i-4);
                        found = true;
                    }
                }
            }
    ...
    public static class Flush implements Combinations{
        ...
        public long calculateScore(Card[] arr) {
            ...
            for (int i = 0; i < arr.length; i++) {
                counterArray[arr[i].getSuit().ordinal()]++;
            }

            for (int i = 0; i < counterArray.length; i++) {
                if (counterArray[i] >= 5) {
                    return 5_00_00_00_00_00L + new HighCard().calculateScore(arr);
                }
            }
    ...
    public static class FullHouse implements Combinations{
        ...
        public long calculateScore(Card[] arr) {
            ...
            if (foundTriple) {
                for (int i = counterArr.length - 1; i >= 2; i--) {
                    if (counterArr[i] >= 2) {
                        return 6_00_00_00_00_00L + (1_00_00_00_00L*tripleIndex) + (1_00_00_00L*i);
                    }
                }
            }
    ...
    public static class FourOfKind implements Combinations{
        ...
        public long calculateScore(Card[] arr) {
            ...
            for (int i = counterArray.length - 1; i >= 2; i--) {
                ...
                if (counterArray[i] == 4) {
                    score += 7_00_00_00_00_00L + (1_00_00_00_00L*i);
                    found = true;
                }
            }
    ...
    public static class StraightFlush implements Combinations{
        ...
        public long calculateScore(Card[] arr) {
            ...
            for (int i = 0; i < arr.length; i++) {
                suitsCounters[arr[i].getSuit().ordinal()][arr[i].getValue()]++;
                if (arr[i].getValue() == 14) {
                    suitsCounters[arr[i].getSuit().ordinal()][1]++;
                }
            }
            ...
            for (int i = 0; i < 4; i++) {
                for (int k = suitsCounters[i].length - 1; k >= 1; k--) {
                    if (suitsCounters[i][k] == 1) {
                        ...
                        for (int d = k; d > k-5; d--) {
                            if (d < 0 || suitsCounters[i][d] < 1) {
                                isStraight = false;
                            }
                        }
                        if (isStraight) {
                            return 8_00_00_00_00_00L + new HighCard().calculateScore(arr);
                        }
                    }
                }
            }
